...
void BSPTree::build()
{
  ...
  if (true)
  {
    ...
    cout << objects.size() << " | " << bounds.min.x << ", " << bounds.min.y << ", " << bounds.min.z << " || " << bounds.max.x << ", " << bounds.max.y << ", " << bounds.max.z << " % " << axis << " X " << axisRetries << endl;
  }
      ...
      rightObjects.size() != objects.size())
  {
    ...
    left = new BSPTree(depth + 1, newAxis, leftObjects);
    right = new BSPTree(depth + 1, newAxis, rightObjects);
  }
...
char BSPTree::toggleAxis()
{
  return axis == 'x' ? 'y' : (axis == 'y' ? 'z' : 'x');
}

Intersection BSPTree::getClosestIntersection(const Ray &ray)
{
  double distance;
...
Intersection BSPTree::getClosestObjectIntersection(const Ray &ray)
{
